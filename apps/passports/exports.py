...
def _build_rows(passports, display_currency, show_vendor, show_passport_number, show_description):
    ...
    if show_passport_number:
        headers.append('Passport No')
    if show_description:
    ...
    for i, passport in enumerate(passports, 1):
        amount = get_display_amount(
            passport.total_amount, passport.currency, passport.exchange_rate, display_currency
        )

        running_balance = apply_balance_step(running_balance, passport.payment_type, amount)

        row = [i, str(passport.issued_date), passport.invoice_no]
        if show_vendor:
            row.append(passport.vendor.name)
        row.append(passport.client_name)
        if show_passport_number:
            row.append(passport.passport_number or '—')
        if show_description:
            row.append(passport.description or '—')
        row += [float(amount), passport.payment_type, float(running_balance)]
        data_rows.append(row)

    return headers, data_rows


def generate_excel(passports, display_currency, meta, options=None):
    options = options or {}
    show_vendor = options.get('show_vendor', True)
    show_passport_number = options.get('show_passport_number', True)
    show_description = options.get('show_description', False)
    ...
    headers, data_rows = _build_rows(
        passports, display_currency, show_vendor, show_passport_number, show_description
    )
...
def generate_pdf(passports, display_currency, meta, options=None):
    options = options or {}
    show_vendor = options.get('show_vendor', True)
    show_passport_number = options.get('show_passport_number', True)
    show_description = options.get('show_description', False)
    ...
    elements.append(Paragraph('Passport Report', title_style))
