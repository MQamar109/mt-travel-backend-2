...
def generate_passport_invoice(passport, display_currency, optional_fields, billing=None):
    ...
    rows = [
        [_para('Invoice No', s_label),   _para(passport.invoice_no,  s_value)],
        [_para('Client',     s_label),   _para(passport.client_name, s_value)],
    ]
    if passport.passport_number:
        rows.append([_para('Passport No', s_label), _para(passport.passport_number, s_value)])
    if 'issued_date' in optional_fields:
        rows.append([_para('Issued Date',   s_label), _para(str(passport.issued_date), s_value)])
    if 'payment_type' in optional_fields:
        rows.append([_para('Payment Type',  s_label), _para(passport.payment_type,     s_value)])
    ...
    amount = _cv(passport.amount, passport.currency, passport.exchange_rate, display_currency)
    total = _cv(passport.total_amount, passport.currency, passport.exchange_rate, display_currency)

    item_rows = [
        [th('Description'), th(f'Amount ({display_currency})')],
        [td('Passport Service' + (f' — {passport.passport_number}' if passport.passport_number else '')), tdr(_fmt(amount))],
        ['', ''],
    ]
    item_rows[2] = [
    ...
    n = len(item_rows)
    items_tbl = Table(item_rows, colWidths=[None, 5 * cm])
    ...
    if 'description' in optional_fields and passport.description:
        el.append(Spacer(1, 14))
        el.append(_para('<b>Note:</b>', s_bold))
        el.append(Spacer(1, 4))
        el.append(_para(passport.description, s_normal))
