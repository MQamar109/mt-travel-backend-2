...
def generate_visa_invoice(visa, display_currency, optional_fields, billing=None):
    """
    ...
    rows = [
        [_para('Invoice No', s_label),   _para(visa.invoice_no,  s_value)],
        [_para('Client',     s_label),   _para(visa.client_name, s_value)],
    ]
    if visa.visa_number:
        rows.append([_para('Visa No', s_label), _para(visa.visa_number, s_value)])
    if 'issued_date' in optional_fields:
        rows.append([_para('Issued Date',   s_label), _para(str(visa.issued_date), s_value)])
    if 'payment_type' in optional_fields:
        rows.append([_para('Payment Type',  s_label), _para(visa.payment_type,     s_value)])
    ...
    amount = _cv(visa.amount, visa.currency, visa.exchange_rate, display_currency)
    total = _cv(visa.total_amount, visa.currency, visa.exchange_rate, display_currency)

    item_rows = [
        [th('Description'), th(f'Amount ({display_currency})')],
        [td('Visa Service' + (f' — {visa.visa_number}' if visa.visa_number else '')), tdr(_fmt(amount))],
        ['', ''],
    ]
    item_rows[2] = [
    ...
    if 'description' in optional_fields and visa.description:
        el.append(Spacer(1, 14))
        el.append(_para('<b>Note:</b>', s_bold))
        el.append(Spacer(1, 4))
        el.append(_para(visa.description, s_normal))
