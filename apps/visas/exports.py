...
def _build_rows(visas, display_currency, show_vendor, show_visa_number, show_description):
    ...
    for i, visa in enumerate(visas, 1):
        amount = get_display_amount(
            visa.total_amount, visa.currency, visa.exchange_rate, display_currency
        )

        running_balance = apply_balance_step(running_balance, visa.payment_type, amount)

        row = [i, str(visa.issued_date), visa.invoice_no]
        if show_vendor:
            row.append(visa.vendor.name)
        row.append(visa.client_name)
        if show_visa_number:
            row.append(visa.visa_number or '—')
        if show_description:
            row.append(visa.description or '—')
        row += [float(amount), visa.payment_type, float(running_balance)]
        data_rows.append(row)

    return headers, data_rows


def generate_excel(visas, display_currency, meta, options=None):
    options = options or {}
    show_vendor = options.get('show_vendor', True)
    show_visa_number = options.get('show_visa_number', True)
    ...
    headers, data_rows = _build_rows(
        visas, display_currency, show_vendor, show_visa_number, show_description
    )
...
def generate_pdf(visas, display_currency, meta, options=None):
    options = options or {}
    show_vendor = options.get('show_vendor', True)
    show_visa_number = options.get('show_visa_number', True)
    show_description = options.get('show_description', False)
    ...
    elements.append(Paragraph('Visa Report', title_style))
